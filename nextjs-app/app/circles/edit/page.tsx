...
const validateSelection = (value: string, otherValue: string, errorMessage: string, setErrorMessage: (message: string) => void) => {
  if (value === otherValue) {
...
const CircleEdit = () => {
  ...
  const handleChangeTeacher = (value: string, isFirst: boolean) => {
    if (isFirst) {
      setTeacher1(value);
      validateSelection(value, teacher2, '講師1と講師2は異なる値を選択してください', setErrorMessage1);
    } else {
      setTeacher2(value);
      validateSelection(value, teacher1, '講師1と講師2は異なる値を選択してください', setErrorMessage1);
    }
  };
  ...
  const handleChangeStudent = (value: string, isFirst: boolean) => {
    if (isFirst) {
      setStudent1(value);
      validateSelection(value, student2, '代表と副代表は異なる値を選択してください', setErrorMessage2);
    } else {
      setStudent2(value);
      validateSelection(value, student1, '代表と副代表は異なる値を選択してください', setErrorMessage2);
    }
  };
  ...
  return <VStack gap={0} w="full" h="full">
    ...
    <VStack p="md" maxW="5xl" m="auto" gap="lg" flexGrow={1}>
      <Heading as="h1" size="lg">サークル編集</Heading>
      <form onSubmit={e => e.preventDefault()}>
        <VStack px="md">
          ...
          <FormControl display="flex" flexDirection={{base: 'row', md: 'column'}} gap={{base: '2xl', md: 'md'}} maxW="2xl">
            ...
            <Select w={{base: 'md', md: 'full'}} placeholder="講師を選択" items={teachers} onChange={(value) => handleChangeTeacher(value, true)} />
          </FormControl>
          <FormControl display="flex" flexDirection={{base: 'row', md: 'column'}} gap={{base: '2xl', md: 'md'}} maxW="2xl">
            <Label flexGrow={1}>講師２</Label>
            <Select w={{base: 'md', md: 'full'}} placeholder="講師を選択" items={teachers} onChange={(value) => handleChangeTeacher(value, false)} />
          </FormControl>
          ...
          <FormControl display="flex" flexDirection={{base: 'row', md: 'column'}} gap={{base: '2xl', md: 'md'}} maxW="2xl">
            ...
            <Select w={{base: 'md', md: 'full'}} placeholder="代表を選択" items={students}  onChange={(value) => handleChangeStudent(value, true)} />
          </FormControl>
          <FormControl display="flex" flexDirection={{base: 'row', md: 'column'}} gap={{base: '2xl', md: 'md'}} maxW="2xl">
            <Label flexGrow={1}>副代表</Label>
            <Select w={{base: 'md', md: 'full'}} placeholder="副代表を選択" items={students}  onChange={(value) => handleChangeStudent(value, false)} />
          </FormControl>
